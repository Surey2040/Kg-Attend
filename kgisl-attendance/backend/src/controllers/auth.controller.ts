...
export const resetDevicesHandler = async (_req: Request, res: Response, next: NextFunction) => { try { const { PrismaClient } = require('@prisma/client'); const prisma = new PrismaClient(); await prisma.student.updateMany({ data: { deviceId: null } }); res.json({ message: 'All student device bindings have been cleared successfully.' }); } catch (error) { next(error); } };
